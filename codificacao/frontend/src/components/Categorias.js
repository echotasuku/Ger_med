    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            if (modoEdicao && categoriaParaEdicao) {
                await axios.put(`http://localhost:8000/api/categorias/${categoriaParaEdicao.id}`, novaCategoria, {
                    headers: {
                        Authorization: `Bearer ${token}` // Adiciona o token no cabeçalho
                    }
                });
            } else {
                await axios.post('http://localhost:8000/api/categorias', novaCategoria, {
                    headers: {
                        Authorization: `Bearer ${token}` // Adiciona o token no cabeçalho
                    }
                });
